...
const ReservasForm = () => {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                const solicitacaoResponse = await fetch(url + `/listaSolicitacao/${userContext.user.saram}`);
                if (!solicitacaoResponse.ok) {
                    throw new Error("Erro ao buscar solicitacao");
                }
                const solicitacaoData = await solicitacaoResponse.json();

                const reservaResponse = await fetch(url + `/listaReserva/${userContext.user.saram}`);
            ...
            >
                <ul>
                    {dataSolicitacao.map((item) => (
                        <Card sx={{ maxWidth: 330, marginBottom: 5, marginTop: 2 }} key={item.id}>
                            <CardMedia
                                sx={{ height: 140 }}
